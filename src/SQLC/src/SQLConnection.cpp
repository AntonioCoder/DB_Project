...
Exit:
	this->~SQLConnection();
	fwprintf(stderr, L"Failed ro connect");
	return false;
}
...
void SQLConnection::Run()
{
	...
	while (_fgetts(wszInput, SQL_QUERY_SIZE - 1, stdin))
	{
		...
		RetCode = SQLExecDirect(m_hStmt, wszInput, SQL_NTS);
...
void SQLConnection::DisplayResults(HSTMT       hStmt,
								   SQLSMALLINT cCols)
{
	...
	do
	{
		...
		if (iCount++ >= m_Height - 2)
		{
			...
			while (!fEnterReceived)
			{
				...
				wprintf(L"   Press ENTER to continue, Q to quit (height:%hd)", m_Height);
...
void SQLConnection::AllocateBindings(HSTMT         hStmt,
									 SQLSMALLINT   cCols,
									 BINDING       **ppBinding,
...
void SQLConnection::DisplayTitles(HSTMT     hStmt,
	DWORD     cDisplaySize,
	BINDING   *pBinding)
{
...
void SQLConnection::SetConsole(DWORD dwDisplaySize,
	BOOL  fInvert)
{
	HANDLE                          hConsole;
...
void SQLConnection::HandleDiagnosticRecord(SQLHANDLE      hHandle,
										   SQLSMALLINT    hType,
										   RETCODE        RetCode)
{
	SQLSMALLINT iRec = 0;
