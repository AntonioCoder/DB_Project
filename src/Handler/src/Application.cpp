...
#else
constexpr wchar_t WINDOW_CLASS_NAME[] = L"WindowClass";
...
void Application::Destroy()
{
	if (gs_pSingelton)
	{
		assert(gs_Windows.empty() &&
			gs_WindowByName.empty() &&
...
//-----------------------------------------------------------------------------------------------
std::shared_ptr<Window> Application::CreateWindow(const std::wstring& windowName,
...
//------------------------------------------------------------------------------------------------
int Application::Run(std::shared_ptr<ViewBase> pAnim)
{
	MSG msg = { 0 };
	...
	if (pWindow)
	{
		...
		case WM_KEYDOWN:
		{
			...
			if (PeekMessage(&charMsg, hwnd, 0, 0, PM_NOREMOVE) && charMsg.message == WM_CHAR)
			{
				...
				c = static_cast<unsigned>(charMsg.wParam);
			}
		...
		case WM_KEYUP:
		{
			...
			KeyCode::Key key = (KeyCode::Key)wParam;
			unsigned c = 0;
			unsigned scanCode = (lParam & 0x00FF0000) >> 16;
			...
			if (int result = ToUnicodeEx(static_cast<UINT>(wParam), scanCode,
				keyboardState, translatedCharacters,
				4, 0, NULL) > 0)
			{
